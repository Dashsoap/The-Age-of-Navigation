...
// components: ["ResourceBuildingCountsComponent"]
...
import {ResourceBuildingCountsComponent, ID as ResourceBuildingCountsComponentID} from "components/ResourceBuildingCountsComponent.sol";
...
uint256 constant ID = uint256(keccak256("system.SettleResourceBuilding"));
...
contract SettleResourceBuildingSystem is System {
    constructor(
        IWorld _world,
        address _components
    ...
    function execute(bytes memory args) public returns (bytes memory) {
        SettleInfo memory settleInfo = abi.decode(args, (SettleInfo));
        return executeTyped(settleInfo);
    }

    function executeTyped(
        SettleInfo memory settleInfo
    ) public returns (bytes memory) {
        uint256 entityId = addressToEntity(msg.sender);

        //verify belonging & charging finished
        //TODO: verify path.length == 0 and in range & area in area
        ResourceBuildingCountsComponent countsComponent = ResourceBuildingCountsComponent(
            getAddressById(components, ResourceBuildingCountsComponentID)
        );
        ...
        countsComponent.set(entityId, countsComponent.getValue(entityId) - 1);
        // }
