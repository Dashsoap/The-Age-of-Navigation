...
// components: ["GuildCrestComponent", "GuildCrestPendingComponent", "GuildCrestGuildComponent", "GuildDisplayComponent"]
...
uint256 constant ID = uint256(keccak256("system.ApproveGuild"));

struct Info {
    uint256 guildCrestId;
}

contract ApproveGuildSystem is System {
    constructor(
        IWorld _world,
        address _components
    ...
    function executeTyped(
        ...
        require(guildCrestComponent.has(guildCrestId) && guildCrestPending.has(guildCrestId), "crest invalid");
        ...
        guildCrestComponent.set(guildCrestId, GuildCrest({ level: 1,
            name: guildConfig.initNameForLevel1,
            contribute: 0,
            allocate: 0,
            createTime: uint64(block.timestamp)
        }));
        guildCrestPending.remove(guildCrestId);
        uint256 targetId = GuildCrestPlayerComponent(getAddressById(components, GuildCrestPlayerComponentID)).getValue(guildCrestId);
        ...
        if (!guildDisplayComponent.has(targetId)) {
            guildDisplayComponent.set(targetId, guildCrestId);
        }
    }
