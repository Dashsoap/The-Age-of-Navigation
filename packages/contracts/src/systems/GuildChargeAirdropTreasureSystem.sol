...
// components: ["TreasureAirdropChargingComponent", "TreasureTimerComponent", "ChargingComponent"]
...
uint256 constant ID = uint256(keccak256("system.GuildChargeAirdropTreasure"));

struct ChargeTreasureInfo {
    uint256 treasureId;
    CoordVerifyParam position;
    CoordVerifyParam target;
    uint32 area;
    uint256 guildCrestId;
}

contract GuildChargeAirdropTreasureSystem is System {
    constructor(
        IWorld _world,
        address _components
    ...
    function executeTyped(
        ...
        uint256 entityId = checkGuildCrest(chargeTreasureInfo.treasureId, chargeTreasureInfo.guildCrestId);
        ...
        require(checkCoordZK(chargeTreasureInfo.target, components), "target invalid");
        ...
        // set TreasureAirdropCharging
        TreasureAirdropChargingComponent(
            getAddressById(components, TreasureAirdropChargingComponentID)
        ).set(chargeTreasureInfo.treasureId, TreasureAirdropCharging({coordHash: chargeTreasureInfo.target.realHash,
            area: uint64(treasureEffect.area * guildConfig.basicAreaFactor / 1000)
        }));
